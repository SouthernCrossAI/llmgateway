...
export async function handleInvoicePaymentSucceeded(event: {
	...
	if (isInitialDevPlanSubscription && initialDevPlanTier) {
		...
		// Any other chat-plan invoice (e.g. the `subscription_update` invoice from
		// a tier upgrade, which the change-tier endpoint already records
		// synchronously). Leave credits untouched and do not fall through to the
		// Pro-subscription handler, which would wrongly flip the org to the Pro plan.
		logger.info(
